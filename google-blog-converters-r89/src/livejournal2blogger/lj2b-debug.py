...
class LiveJournal2Blogger(object):
  ...
  def __init__(self, username, password, server='livejournal.com'):
    ...
    logging.error("Request from user %s" % username)
  ...
  def _GetPosts(self):
    ...
    try:
      ...
      self.challenge, self.challenge_response = self._GetAuthTokens()
      logging.error('Retrieving auth tokens: %d ms' % ((time.time() - start_time) * 1000))
    except:
      logging.error(traceback.format_exc())

    logging.error("%s %s" % (self.challenge, self.challenge_response))
    ...
    while num_failures < max_failures:
      ...
      try:
        ...
        logging.error('Sync-ing %d items: %d ms' %
...
#               continue
            ...
            try:
              ...
              event = self.server.LJ.XMLRPC.getevents({
                  ...
                  'auth_challenge': self.challenge,
                  'auth_response': self.challenge_response})
              logging.error('Retrieved item %s: %d ms' %
  ...
  def _GetAuthTokens(self):
    ...
    """
    # TODO(jlueck):  Add in logic to only recalculate the challege if it has
    #                expired.
    response = self.server.LJ.XMLRPC.getchallenge()
    challenge = response['challenge']
    print ("Auth tokens are available for %d seconds" %
           (response['expire_time'] - response['server_time']))
